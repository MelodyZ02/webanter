...
$stmt = $db->prepare("SELECT * FROM userinfo INNER JOIN chat ON userinfo.userID = chat.receiverID WHERE chat.senderID = :uid;");
...
<div class="card text-center border-dark mb-3 mx-auto bg-light rounded-top" style="width: 50rem;">
    ...
    <div class="card-body">
        <?php foreach ($a as $receiver): ?>
            <div class="card text-left border-dark mb-3" style="width: auto;">
                <div class="card-body text-dark">
                    <form method="post" action="">
                    <td class="card-body"><img src="<?= $receiver['profileIMG'] ?>" alt="" style="height: 50px; width: 50px; border-radius: 50%;"> <?= $receiver['name']?></td>

                        <input name="FID" type="hidden" value="<?=$friend['FID']?>">
                        <input name="userID" type="hidden" value="<?=$_SESSION['user']?>">
                        <td><input type="submit" class="btn btn-info" align="right" value="<?= $lang['chat'] ?>"></td>
                    </form>
                </div>
            </div>
        <?php endforeach;?>
